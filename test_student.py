...
async def main():
    async with httpx.AsyncClient(base_url="http://127.0.0.1:8000/api/v1") as client:
        # Register a student
        res = await client.post("/auth/register", json={"name": "Test Student", "email": "teststu@test.com", "password": "password123", "role": "student"})
        ...
        for exam in exams:
            ...
            res2 = await client.get(f"/exams/{exam['id']}/student", headers=headers)
            print("GetForStudent:", res2.status_code)
